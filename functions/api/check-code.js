...
export async function onRequestPost(context) {
  try {
    const { code, action } = await context.request.json()
    if (!code || !code.trim()) return Response.json({ status: 'empty' }, { headers: CORS })

    const codeUpper = code.toUpperCase().trim()
    if (!VALID_CODES.has(codeUpper)) return Response.json({ status: 'invalido' }, { headers: CORS })

    const usageStr = await context.env.CODE_USAGE.get(codeUpper)
    const usage = usageStr ? parseInt(usageStr) : 0
    if (usage >= 2) return Response.json({ status: 'agotado' }, { headers: CORS })

    if (action === 'usar') {
      await context.env.CODE_USAGE.put(codeUpper, String(usage + 1))
    }

    return Response.json({ status: 'valido' }, { headers: CORS })
  } catch {
